// mall.c -- uses the Queue interface
...
int main(void)
{
    ...
    long turnaways1 = 0;        // turned away by full queue
    ...
    srand((unsigned int)time(NULL));    // random initializing of rand()
    ...
    {
        // Process the new comer for queue 1
        ...
        // Process the new comer for queue 2
        if (newcustomer(min_per_cust))
        {
        ...
        // Process the current customer who is consulting
        // If the consultation time has reached, kick him away
        ...
        // Consultation time hasn't be reached, decrease wait_time
        if (wait_time1 > 0)
            wait_time1--;
        if (wait_time2 > 0)
            wait_time2--;
        // Record every minute the number of customers
...
// set to when and the processing time est to a random value
// in the range 1 - 3
