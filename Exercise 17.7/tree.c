...
bool AddItem(const Item * pi, Tree * ptree)
{
    ...
    look = SeekItem(pi, ptree); /* search the item */
    if (look.child != NULL)     /* if found, increment the frequency */
    {
        (look.child->item.freq)++;
...
/* Make use of the SeekItem() function */
void SearchWord(Tree * ptree, void (* pfun)(Item item))
{
    char temp[SLEN];
    Item tmpItem;
    Pair look;
    puts("Enter the word you want to search: ");
    scanf("%20s", temp);
    while (getchar() != '\n')
        continue;
    tmpItem.word = temp;
    tmpItem.freq = 0;
    look = SeekItem(&tmpItem, ptree);
    if (!(look.child))
        puts("The word is not found!\n");
    else
    {
        tmpItem = look.child->item;
        (*pfun)(tmpItem);
    }
}
...
//    else                                /* can allow duplicates now  */
//        root->item->freq++;             /* the word is found, increment its frequency */
}
...
static Pair SeekItem(const Item * pi, const Tree * ptree)
{
    ...
    if (look.child == NULL)
        return look;                /* early return, tree is empty  */
...
/* ptr is address of parent member pointing to target node  */
{
    ...
    {
        /* find where to reattch right subtree  */
